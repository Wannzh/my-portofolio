      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            {/* CHANGED: Teks di-hardcode diganti */}
            <span className="text-[#e0e0e0] font-mono">{t('certificates.filterBy')}</span>
          </div>
        ...
        {loading ? (
          ...
          <>
            ...
            >
              {filteredCertificates.map((cert: Certificate) => (
                ...
                >
                  ...
                  {/* Content */}
                  <h3 className="text-lg font-bold text-[#ffffff] mb-2">{cert.title}</h3>
                  <p className="text-[#e0e0e0] text-sm mb-2">{cert.issuer}</p>
                  <p className="text-[#b0b0b0] text-sm mb-3 font-mono">{cert.year}</p>
                  <p className="text-[#e0e0e0] text-sm mb-4 flex-1">{cert.description}</p>
                  ...
                  >
                    <Download size={16} />
                    <span className="text-sm">{t('certificates.viewCertificate')}</span>
                  </button>
                </motion.div>
              ))}
            </motion.div>
            ...
            {filteredCertificates.length === 0 && (
              ...
              >
                <p className="text-[#b0b0b0] text-lg">{t('certificates.noCertificates')}</p>
              </motion.div>
            )}
          </>
        )}
